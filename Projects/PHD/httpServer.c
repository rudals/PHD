...
uint8_t schedule_num = 0;
SCHEDULE_LIST_TYPE schedule_list[MAX_SCHEDULE_NUM];
...
uint8_t http_post_cgi_handler(uint8_t * uri_name, st_http_request * p_http_request, uint8_t * buf, uint32_t * file_len);uint8_t http_get_cgi_handler(uint8_t * uri_name, uint8_t * buf, uint32_t * file_len);
...
int get_empty_schedule()
{
	...
	for(i=0 ; i<MAX_SCHEDULE_NUM ; i++) {
		if(!schedule_list[i].func) break;
	}
	if(i<MAX_SCHEDULE_NUM) return i;
...
uint8_t http_post_cgi_handler(uint8_t * uri_name, st_http_request * p_http_request, uint8_t * buf, uint32_t * file_len)
{
	...
	uint8_t func = 0;
	uint8_t repeat = 0;
	time_t unix_time = 0UL;
	uint8_t i = 0;
	char *endptr;
	int index = -1;

	const uint8_t* uri = p_http_request->URI;

	if(strcmp((const char *)uri_name, "phd_schedule.cgi") == 0) {
		if((param = get_http_param_value((char *)uri, "func"))) {
			func = atoi(param);

			if(func){
				index = get_empty_schedule();
				if(index == -1) {
					ret_val = 0;
					len = sprintf((char *)buf, "%d", ret_val);
					return 0;
				}
				schedule_list[index].func = func;

				if((param = get_http_param_value((char *)uri, "content"))) {
					strncpy(schedule_list[index].content, param, 31);
							schedule_list[index].content[32] = '\0';

					if((param = get_http_param_value((char *)uri, "date"))) {
						unix_time = strtoull(param, &endptr, 10);
						schedule_list[index].unix_time = unix_time;
						
						struct tm *t = localtime(&unix_time);
						memcpy(&schedule_list[index].stime, t, sizeof(struct tm));

						if((param = get_http_param_value((char *)uri, "date_str"))) {
							strncpy(schedule_list[index].date_str, param, 16);
							schedule_list[index].date_str[17] = '\0';

							if((param = get_http_param_value((char *)uri, "repeat"))) {
								repeat = atoi(param);
								schedule_list[index].repeat = repeat;
							}
						}
					}

					if(schedule_num < MAX_SCHEDULE_NUM) schedule_num++;
				}
			} else {
				if((param = get_http_param_value((char *)uri, "date"))) {
					uint8_t del_index = atoi(param) - 1;
					for(int i=del_index ; i<MAX_SCHEDULE_NUM ; i++) {
						if(i == MAX_SCHEDULE_NUM - 1){
							memset(&schedule_list[i], 0x0, sizeof(SCHEDULE_LIST_TYPE));
						} else {
							memcpy(&schedule_list[i], &schedule_list[i+1], sizeof(SCHEDULE_LIST_TYPE));
							memset(&schedule_list[i+1], 0x0, sizeof(SCHEDULE_LIST_TYPE));
						}
					}
