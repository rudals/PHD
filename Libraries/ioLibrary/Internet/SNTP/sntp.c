...
int8_t SNTP_run(time_t *time)
{
	...
	case SOCK_UDP:
		if ((RSR_len = getSn_RX_RSR(NTP_SOCKET)) > 0)
		{
			...
			unsigned long long ntp_time = (unsigned long long)data_buf[43] |
										((unsigned long long)data_buf[42] << 8) |
										((unsigned long long)data_buf[41] << 16) |
										((unsigned long long)data_buf[40] << 24);
			time_t unix_time = ntp_time - NTP_TIMESTAMP_DELTA + (9 * 3600);
			*time = unix_time;
